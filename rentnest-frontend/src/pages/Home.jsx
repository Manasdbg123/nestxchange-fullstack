        ...
        <>
                    ...
                    <div className="flex flex-col items-start gap-10 lg:flex-row lg:items-center lg:justify-between">
                        <div className="max-w-xl">
                            ...
                            <h1 className="mt-6 font-display text-4xl font-extrabold leading-tight text-white sm:text-5xl">
                                Rent your next home
                                <span className="block text-brand-300">straight from the owner</span>
                            </h1>

                            <p className="mt-5 text-lg leading-relaxed text-ink-300">
                                No agents in the middle, no month's rent lost to brokerage. Search verified
                                listings, talk to owners directly, and book a visit when you are ready.
                            </p>
                        ...
                        <div className="w-full lg:max-w-xl">
                            <HeroSearch suggestedCities={cities} />
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="border-b border-ink-200 bg-white dark:border-ink-800 dark:bg-ink-950">
                <div className="container-page flex gap-3 overflow-x-auto py-5 hide-scrollbar">
                    {PROPERTY_CATEGORIES.filter((entry) => entry.id !== 'all').map((entry) => (
                        <Link
                            key={entry.id}
                            to={`/search?${entry.types.map((type) => `type=${type}`).join('&')}`}
            ...
            {/* -------------------------------------------------------- Browse */}
            <section className="container-page py-16">
                <SectionHeading align="left" eyebrow="Explore" title="Browse rentals across India" />
                <div className="mt-8">
                    <BrowseLinks
                        cities={(cities.length ? cities : ['Bengaluru', 'Mumbai', 'Pune', 'Delhi', 'Hyderabad']).map(
                            (city) => ({
                                label: `Rentals in ${city}`,
                                to: `/search?city=${encodeURIComponent(city)}`,
                            }),
                        )}
                        types={[
                            { label: 'Flats for rent', to: '/search?type=APARTMENT' },
                            { label: 'Independent houses', to: '/search?type=HOUSE' },
                            { label: 'Villas for rent', to: '/search?type=VILLA' },
                            { label: 'Studio apartments', to: '/search?type=STUDIO' },
                            { label: 'PG & hostels', to: '/search?type=PG&type=ROOM' },
                            { label: 'Shops & offices', to: '/search?type=SHOP&type=COMMERCIAL' },
                            { label: 'Fully furnished homes', to: '/search?furnishing=FULLY_FURNISHED' },
                            { label: 'Under ₹20,000', to: '/search?maxRent=20000' },
                        ]}
                    />
                </div>
            </section>
                ...
                <div className="container-page">
                    ...
                    <div className="mt-10">
                        <Faq
                            items={[
                                {
                                    question: 'Is RentNest really free for tenants?',
                                    answer:
                                ...
                                {
                                    question: 'How does RentNest make money then?',
                                    answer:
            ...
            <section className="container-page pb-8">
                <CtaBanner
                    ...
                    secondary={{ to: '/search', label: 'Browse rentals instead' }}
                />
            </section>
        </>
    );
